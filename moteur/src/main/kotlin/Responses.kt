...
data class Response(
    ...
    data class Response7(
        ...
        enum class Frequency(val value: String, val weight: Int) {
            UNKNOWN("Ne connais pas", 0),
            NEVER("Jamais", 1),
            LOW("Un peu", 2),
            REGULARLY("Régulièrement", 3),
            ALWAYS("Tout le temps", 4);
    ...
    {
        enum class Interested(val value: String, val weight: Int) {
            IDK("Ne connais pas", 0),
            IDC("Pas intéressé", 1),
            INTERESTED("Interessé", 2),
            ALREADY_MEMBER("J'y suis", 3);
